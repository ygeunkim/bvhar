...
Eigen::VectorXd hs_coef(Eigen::VectorXd response_vec, Eigen::MatrixXd design_mat, double prior_var, Eigen::MatrixXd shrink_mat, int coef_type) {
  ...
  if (coef_type == 2) {
    ...
    Eigen::VectorXd lin_solve = (design_mat * shrink_mat * design_mat.transpose() + Eigen::MatrixXd::Identity(sample_size, sample_size)).llt().solve(
      response_vec / sqrt(prior_var) - design_mat * gaussian_u - gaussian_delta);
...
Rcpp::List estimate_sur_horseshoe(int num_iter, int num_burn, Eigen::MatrixXd x, Eigen::MatrixXd y,
  ...
  for (int i = 1; i < num_iter; i++) {
    ...
    coef_record.row(i) = hs_coef(response_vec, design_mat, sig_record(i - 1, 0), lambda_mat, coef_type);
    // 2. sigma (variance)
