...
protected:
  ...
  int num_coef;
	...
	void updateSv() { sqrt_sv = diag_vec.cwiseSqrt().transpose().replicate(num_design, 1); }
...
public:
	MinnReg(const MinnParams<RegParams>& params, const LdltInits& inits, unsigned int seed) : McmcReg(params, inits, seed) {
