...
#' 
#' @param spike_sd Standard deviance for Spike normal distribution (in the form of coefficient matrix).
#' @param slab_sd Standard deviance for Slab normal distribution (in the form of coefficient matrix).
#' @param prop_sparse Bernoulli parameter for sparsity proportion (in the form of coefficient matrix).
#' @details 
...
#' 
#' - `spike_sd` sets \eqn{\kappa_{0j}} in the form of matrix
#' - `slab_sd` sets \eqn{\kappa_{1j}} in the form of matrix
#' 
#' \eqn{\gamma_j} is the proportion of the nonzero coefficients and it follows
#' \deqn{\gamma_j \sim Bernoulli(q_j)}
#' 
#' - `prop_sparse` sets \eqn{q_j} in the form of matrix
#' 
#' @references 
...
set_spikeslab_coef <- function(spike_sd = NULL, slab_sd = NULL, prop_sparse = NULL) {
  if (!identical(dim(spike_sd), dim(slab_sd))) {
    stop("The dimensions of 'spike_sd' and 'spike_sd' should be the same.")
  }
  if (ncol(prop_sparse) != ncol(spike_sd)) {
    stop("The dimension of 'prop_sparse' should be the same as the coefficient matrix.")
  }
  coef_param <- list(
    coef_spike = spike_sd,
    coef_slab = slab_sd,
...
#' @details 
#' For precision matrix \eqn{\Sigma^{-1}}, SSVS applies Cholesky decomposition in MCMC.
#' \deqn{\Sigma^{-1} = \Psi \Psi^T}
#' where \eqn{\Psi = \{\psi_{ij}\}} is upper triangular.
#' 
#' Diagonal components follow the gamma distribution with shape-rate notation.
#' \deqn{\psi_{jj} \sim Gamma(a_j, b_j)}
#' For each row of off-diagonal components, we apply spike-slab prior again.
#' \deqn{\psi_{ij} \mid w_{ij} \sim (1 - w_{ij}) N(0, \kappa_{0,ij}^2) + w_{ij} N(0, \kappa_{1,ij}^2)}
#' \deqn{w_{ij} \sim Bernoulli(q_{ij})}
#' where \eqn{i = 1, \ldots, j - 1} and \eqn{j = 2, \ldots, m} (lower triangular part).
#' 
#' @references 
#' Jochmann, M., Koop, G., & Strachan, R. W. (2010). *Bayesian forecasting using stochastic search variable selection in a VAR subject to breaks*. International Journal of Forecasting, 26(2), 326–347. doi:[10.1016/j.ijforecast.2009.11.002](https://www.sciencedirect.com/science/article/abs/pii/S0169207009001782?via%3Dihub)
#' 
...
#' 
#' @param coef_ss Spike and slab specification for coefficient matrix.
#' @param sig_ss Spike and slab specification for covariance matrix.
#' @details 
#' Setting for MCMC.
#' 
#' @references 
