...
sim_hs_var <- function(bayes_spec, p, dim_data = NULL, include_mean = TRUE) {
  ...
  if (length(bayes_spec$local_sparsity) == 1) {
    bayes_spec$local_sparsity <- rep(bayes_spec$local_sparsity, dim_design)
  } else if (length(bayes_spec$local_sparsity) != dim_design) {
    stop("The length of 'local_sparsity' should be the same with coefficients row.")
  }
  # Minnesota specification-------------
  if (!is.bvharspec(bayes_spec$minn)) {
    stop("Provide 'bvharspec' for 'bayes_spec$minn'.")
  }
  if (bayes_spec$minn$prior != "Minnesota") {
    stop("'bayes_spec$minn' must be the result of 'set_bvar()'.")
  }
  if (is.null(bayes_spec$minn$sigma)) {
    stop("'sigma' in 'set_bvar()' should be specified. (It is NULL.)")
  }
  if (length(bayes_spec$minn$sigma) != dim_data) {
    stop("Length of 'sigma' should be the same as 'dim_data'.")
  }
  ...
  #   matrix(mn_mean, nrow = dim_design, ncol = dim_data),
  #   bayes_spec$global_sparsity^2 * diag(bayes_spec$local_sparsity^2),
  #   bayes_spec$init_cov
  # )
  coef_mat <- minn_simulation$mn
  sig_mat <- minn_simulation$iw
  eigen_root <- 
...
sim_hs_vhar <- function(bayes_spec, har = c(5, 22), dim_data = NULL, include_mean = TRUE) {
  ...
  num_har <- ifelse(include_mean, 3 * dim_data + 1, 3 * dim_data)
  if (length(bayes_spec$local_sparsity) == 1) {
    bayes_spec$local_sparsity <- rep(bayes_spec$local_sparsity, num_har)
  } else if (length(bayes_spec$local_sparsity) != num_har) {
    stop("The length of 'local_sparsity' should be the same with coefficients row.")
  }
  # Minnesota specification------------
  if (!is.bvharspec(bayes_spec$minn)) {
  ...
  if (bayes_spec$minn$process != "BVHAR") {
    stop("'bayes_spec' must be the result of 'set_bvhar()' or 'set_weight_bvhar()'.")
  }
  if (is.null(bayes_spec$minn$sigma)) {
    stop("'sigma' in 'set_bvhar()' or 'set_weight_bvhar()' should be specified. (It is NULL.)")
