    ...
    if (is.paraminit(init_spec$init_coef)
      ...
      if (is.unifinit(init_spec$init_coef)) {
        init_spec$init_coef <- matrix(
          runif(dim_design * dim_data, min = init_spec$init_coef$param[1], max = init_spec$init_coef$param[2]),
          nrow = dim_design,
          ncol = dim_data
        )
      } else {
        init_spec$init_coef <- matrix(init_spec$init_coef$param, nrow = dim_design, ncol = dim_data)
      }
      if (is.unifinit(init_spec$init_coef_dummy)) {
        ...
        init_spec$init_coef_dummy <- matrix(init_spec$init_coef_dummy$param, nrow = dim_design, ncol = dim_data)
      }
      ...
      if (is.unifinit(init_spec$init_chol)) {
        ...
        init_chol[upper.tri(init_chol)] <- runif(
          dim_data * (dim_data - 1) / 2,
          min = init_spec$init_chol$param[1],
          max = init_spec$init_chol$param[2]
        )
        init_spec$init_chol <- init_chol
      } else {
        init_chol <- diag(dim_data)
        init_chol[upper.tri(init_chol)] <- init_spec$init_chol$param
        init_spec$init_chol <- init_chol
      }
      if (is.unifinit(init_spec$init_chol_dummy)) {
        init_chol_dummy <- diag(dim_data)
        init_chol_dummy[upper.tri(init_chol_dummy)] <- rbinom(
          dim_data * (dim_data - 1) / 2,
          1,
          runif(1, 1 / (1 + exp(init_spec$init_chol_dummy$param[2])), 1 / (1 + exp(init_spec$init_chol_dummy$param[1])))
        )
