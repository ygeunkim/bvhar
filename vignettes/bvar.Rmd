...
description: >
  See how to model BVAR using
  `bvar_minnesota()` and `bvar_flat().
...
```{r}
autoplot(est_bvar, type = "coef", var_name = "GVZCLS")
```

Density plot for variance:

```{r}
autoplot(est_bvar, type = "variance")
```
...
```{r}
autoplot(pred_bvar, x_cut = 500)
```

```{r}
(mse_bvar <- mse(pred_bvar, etf_test))
```
...
```{r}
autoplot(pred_ghosh, x_cut = 500)
```

```{r}
(mse_ghosh <- mse(pred_ghosh, etf_test))
```
