...
(pred_bvarmat <- pred_bvar$forecast)
```

```{r}
(mse_bvar <- apply(etf_test - pred_bvarmat, 2, function(x) mean(x^2)))
```
...
```{r}
# VAR----------------------------
fit_var <- var_lm(etf_train, p = 1)
pred_varmat <- predict(fit_var, h)$forecast
mse_var <- apply(etf_test - pred_varmat, 2, function(x) mean(x^2))
# VHAR---------------------------
fit_har <- vhar_lm(etf_train)
pred_harmat <- predict(fit_har, h)$forecast
mse_har <- apply(etf_test - pred_harmat, 2, function(x) mean(x^2))
# BVHAR (original Minnesota)-----
fit_bvhar <- bvhar_minnesota(etf_train, sigma = sig, lambda = lam, delta = delta, eps = eps)
pred_bvharmat <- predict(fit_bvhar, h)$forecast
mse_bvhar <- apply(etf_test - pred_bvharmat, 2, function(x) mean(x^2))
```
...
(fit_ghosh <- bvar_flat(etf_train, bvar_lag, U))
```
...
(pred_ghoshmat <- pred_ghosh$forecast)
```

```{r}
(mse_ghosh <- apply(etf_test - pred_ghoshmat, 2, function(x) mean(x^2)))
```
