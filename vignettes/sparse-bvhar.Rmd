...
hs_data <- 
  sim_var(
    ...
    var_lag = 1,
    sig_error = hs_sig,
    init = matrix(runif(ncol(hs_coef)), nrow = 1, ncol = ncol(hs_coef)),
    method = "chol"
  )
colnames(var_data) <- paste0("y", 1:6)
```
...
```{r hsspec}
(hs_spec <- set_horseshoe(init_local = rep(.2, 7), init_global = .1, cov(hs_data)))
```
...
- `num_warm`: The number of warm-up (burn-in)
- `thinning`: Thinning
- `bayes_spec`: SSVS specification using `set_horseshoe()`.
- `include_mean`: whether including the constant term in the model.
- `verbose`: whether to display the progress bar.
