...
- `num_warm`: The number of warm-up (burn-in)
- `thinning`: Thinning
...
mcmc_trace(var_ssvs$param, regex_pars = "psi")
```

```{r alphadens}
mcmc_dens(var_ssvs$param, pars = c("alpha[1]", "alpha[2]", "alpha[9]"))
```
...
```{r truevharcoef}
vhar_coef <- matrix(0L, nrow = 6 * 3 + 1, ncol = 6)
diag(vhar_coef[1:6,]) <- .2
diag(vhar_coef[7:12,]) <- .2
diag(vhar_coef[13:18,]) <- .2
vhar_coef[19,] <- 1
vhar_coef
```

and the same Cholesky factor with above example:
...
vhar_data <-
  sim_vhar(
    ...
    week = 5,
    month = 22,
    sig_error = solve(vhar_chol %*% t(vhar_chol)),
    method = "chol"
  )
colnames(vhar_data) <- paste0("y", 1:6)
```
...
Consider the similar example for convenience.

```{r hssetting}
# coefficients----------------------------
hsvar_coef <- var_coef
for (i in seq_len(ncol(hsvar_coef))) {
  hsvar_coef[i, -i] <- 1e-3
}
# precision-------------------------------
hs_sig <- solve(var_chol %*% t(var_chol))
```
...
hsvar_data <- 
  sim_var(
    ...
    var_lag = 1,
    sig_error = hs_sig,
    init = matrix(runif(ncol(hsvar_coef)), nrow = 1, ncol = ncol(hsvar_coef)),
    method = "chol"
  )
colnames(var_data) <- paste0("y", 1:6)
```
...
```{r hsspec}
(hs_spec <- set_horseshoe(local_sparsity = rep(.2, 7), global_sparsity = .1, cov(hs_data)))
```
...
```{r hsvharsimul}
# coefficients for VHAR------------
hsvhar_coef <- vhar_coef
for (i in seq_len(ncol(hsvhar_coef))) {
  hsvhar_coef[i, -i] <- 1e-3
}
# simulation-----------------------
hsvhar_data <- 
  sim_vhar(
...
```{r hsvharspec}
(hsvhar_spec <- set_horseshoe(local_sparsity = rep(.2, 3 * ncol(hsvhar_data) + 1), global_sparsity = .1, cov(hsvhar_data)))
```
