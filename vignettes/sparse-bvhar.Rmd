...
If each `bayes_spec` and `init_spec` is not specified, corresponding automatic approach is performed.
...
Since this process is Full Bayesian, every hyperparameter is chosen by Gibbs sampler.
Do not worry about setting each number.

```{r hsspec}
(hs_spec <- set_horseshoe(local_sparsity = .2, global_sparsity = .1, minn = set_bvar()))
```
...
```{r hsvharspec}
(hsvhar_spec <- set_horseshoe(local_sparsity = .2, global_sparsity = .1, minn = set_bvhar()))
```
