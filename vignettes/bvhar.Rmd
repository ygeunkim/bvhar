...
`bvar_minnesota(y, p, sigma, lambda, delta, eps = 1e-04)` fits BVAR(p).
See the Vignettes for BVAR if you want to know the detail of each argument.
...
We call this VAR-type Minnesota prior.

### VAR-type Minnesota

`bvhar_minnesota(y, type = "VAR", sigma, lambda, delta, eps = 1e-04)` fits BVHAR with this prior.
...
```{r}
(fit_bvhar_v1 <- bvhar_minnesota(etf_train, bvhar_spec_v1))
```
