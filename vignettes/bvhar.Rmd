...
```{r}
pred_bvharmat <- pred_bvhar$forecast
...
```{r}
pred_bvhar_othermat <- pred_bvhar_other$forecast
(mse_bvhar_other <- apply(etf_test - pred_bvhar_othermat, 2, function(x) mean(x^2)))
```
...
```{r}
autoplot(pred_bvhar_other, show.legend = FALSE)
```
