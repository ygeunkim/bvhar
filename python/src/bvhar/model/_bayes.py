from ..utils._misc import make_fortran_array, check_np, get_coef_init, build_grpmat, process_record, concat_chain, concat_params, process_dens_forecast, process_dens_spillover, process_dens_vector_spillover, process_dens_list_spillover
...
class _AutoregBayes:
    ...
    def __init__(
        ...
        coef_config = SsvsConfig(),
        contem_config = SsvsConfig(),
    ...
    def _validate(self):
        ...
        if not isinstance(self.coef_spec_, _BayesConfig):
            raise TypeError("`coef_spec` should be the derived class of `_BayesConfig`.")
        if not isinstance(self.contem_spec_, _BayesConfig):
...
class VarBayes(_AutoregBayes):
    ...
    coef_config : '_BayesConfig'
        Prior configuration for coefficient, by default SsvsConfig()
    contem_config : '_BayesConfig'
        Prior configuration for contemporaneous coefficient, by default SsvsConfig()
...
class VharBayes(_AutoregBayes):
    ...
    def __init__(
        ...
        super().__init__(data, month, 3, n_chain, n_iter, n_burn, n_thin, coef_config, contem_config, cov_config, intercept_config, fit_intercept, minnesota, ggl, verbose)
